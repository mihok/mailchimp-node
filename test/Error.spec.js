...
describe('Error', function() {
  ...
  describe('MailchimpError', function() {
    ...
    it('Pulls in headers', function() {
      ...
      var e = Error.MailchimpError.generate({title: 'Invalid Resource', headers: headers});
      expect(e).to.have.property('headers', headers);
    });

    it('Pulls in request IDs', function() {
      var e = Error.MailchimpError.generate({title: 'Invalid Resource', requestId: 'foo'});
      expect(e).to.have.property('requestId', 'foo');
    });

    it('Pulls in HTTP status code', function() {
      var e = Error.MailchimpError.generate({title: 'Invalid Resource', statusCode: 400});
