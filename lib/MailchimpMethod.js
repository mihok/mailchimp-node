  ...
  return function() {
    ...
    return this.wrapTimeout(new Promise((function(resolve, reject) {
      ...
      if (args.length) {
        path = this.createResourcePathWithSymbols(spec.path);
        err = new Error(
          'Mailchimp: Unknown arguments (' + args + '). Did you mean to pass an options ' +
          'object? See https://github.com/mihok/mailchimp-node/wiki/Passing-Options.' +
          ' (on API request to ' + requestMethod + ' `' + path + '`)'
        );
        reject(err);
        return;
...
module.exports = mailchimpMethod;
