...
var signature = {
  ...
  verifyHeader: function(payload, header, secret, tolerance) {
    ...
    if (!details || details.timestamp === -1) {
      throw new Error.MailchimpSignatureVerificationError({
    ...
    if (!signatureFound) {
      throw new Error.MailchimpSignatureVerificationError({
        message: 'No signatures found matching the expected signature for payload.' +
          ' Are you passing the raw request body you received from Mailchimp?' +
          ' https://github.com/mailchimp/mailchimp-node#webhook-signing',
        detail: {
          header: header,
          payload: payload,
        },
