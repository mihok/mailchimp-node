...
MailchimpError.generate = function(rawMailchimpError) {
  // TODO: Since Mailchimp doesnt send a 'code' param like Stripe does, we are
  //  going to inject it, but this may not be the best place for it
  const title = rawMailchimpError.title;
  const code = title.toLowerCase().replace(/\s/, '_');
  ...
  case 'Invalid Resource':
    return new _Error.MailchimpInvalidRequestError(Object.assign(rawMailchimpError, {code: code, message: title}));
  case 'API Key Missing':
  case 'API Key Invalid':
    return new _Error.MailchimpAuthenticationError(Object.assign(rawMailchimpError, {code: code, message: title}));
    // case 'api_error':
